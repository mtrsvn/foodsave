...
<body>
    <div class="dashboard-container">
        ...
        <div class="main-content">
            ...
            <div class="content-pane" style="margin-top: 6px;">
                <div class="filter-header" style="display: flex; align-items: center; justify-content: space-between; padding: 20px 30px; border-bottom: 1px solid #eee; gap: 15px;">
                    
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        <button class="filter-btn <?= $filter == 'All' ? 'active' : '' ?>" data-filter="All">All Items (<?= $counts['All'] ?>)</button>
                        <button class="filter-btn <?= $filter == 'Low Stock' ? 'active' : '' ?>" data-filter="Low Stock">Low Stock (<?= $counts['Low Stock'] ?>)</button>
                        <button class="filter-btn <?= $filter == 'Near Expiry' ? 'active' : '' ?>" data-filter="Near Expiry">Near Expiry (<?= $counts['Near Expiry'] ?>)</button>
                        <button class="filter-btn <?= $filter == 'Expired' ? 'active' : '' ?>" data-filter="Expired">Expired (<?= $counts['Expired'] ?>)</button>
                    </div>
                    ...
                    <div class="notif-search-wrapper" style="position: relative; display: flex; align-items: center; width: 300px;">
                        ...
                        <input type="text" id="notifSearchInput" 
                               ...
                               style="width: 100%; padding: 10px 50px 10px 40px; border-radius: 25px; border: 1px solid #e2e8f0; outline: none;">
                        <button type="button" class="search-circle-btn" onclick="triggerSearch()" 
                                style="position: absolute; right: 5px; width: 35px; height: 35px; border-radius: 50%; background: #8dae84; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center;">
                    ...
                    <form method="GET" id="filterForm" style="display: flex; align-items: center; gap: 12px;">
                        ...
                        <select name="time_range" id="filterSelect" onchange="handleFilterChange(this.value)" style="padding: 8px; border-radius: 8px; border: 1px solid #e2e8f0;">
                        ...
                        <div id="customDateInputs" style="display: <?= ($time_range == 'CUSTOM') ? 'flex' : 'none' ?>; gap: 8px; align-items: center;">
                            <input type="date" name="start_date" value="<?= htmlspecialchars($start_date) ?>" style="padding: 7px; border-radius: 8px; border: 1px solid #e2e8f0;">
                            <span style="color: #64748b;">to</span>
                            <input type="date" name="end_date" value="<?= htmlspecialchars($end_date) ?>" style="padding: 7px; border-radius: 8px; border: 1px solid #e2e8f0;">
                            <button type="submit" style="padding: 7px 12px; background: #8dae84; color: white; border: none; border-radius: 6px; cursor: pointer;"><i class="fas fa-check"></i></button>
                        </div>
                    </form>
                </div>

                <button onclick="markAllAsRead()" style="background: white; border: 1px solid #ef4444; color: #ef4444; padding: 10px 18px; border-radius: 12px; font-weight: 700; cursor: pointer; margin: 15px;">
                    <i class="fas fa-check-double"></i> Mark All as Read
                </button>

                <div style="display: flex; flex: 1; min-height: 0;">
                    <div id="notif-list" style="width: 35%; border-right: 1px solid #eee; display: flex; flex-direction: column;">
                        <div style="flex: 1; overflow-y: auto;">
                            ...
                            <?php else: ?>
                                ...
                                <div class="notif-item <?= $n['category'] ?> <?= $n['is_read'] == 0 ? 'unread' : '' ?>"
                                    ...
                                    <p style="font-size:0.8rem; color:#64748b; margin:5px 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                        <?= htmlspecialchars($n['message']) ?>
                                    </p>
                                    <small style="color:#94a3b8;"><?= date('M d, H:i', strtotime($n['created_at'])) ?></small>
                                </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </div>

                        <div class="pagination-wrapper" style="padding: 15px; border-top: 1px solid #eee; background: #f8fafc;">
                            <div class="pagination-info" style="font-size: 0.75rem; color: #64748b; margin-bottom: 5px;">
                                Showing <b><?= ($total_rows > 0) ? $offset + 1 : 0 ?></b> to <b><?= min($offset + $limit, $total_rows) ?></b> of <b><?= $total_rows ?></b> entries
                            </div>
                            <div class="pagination-controls" style="display: flex; align-items: center; gap: 5px; flex-wrap: wrap;">
                                <?php 
                                    $base_url = "notifications.php?filter=" . urlencode($filter) . (!empty($search) ? "&search=" . urlencode($search) : "") . "&time_range=" . urlencode($time_range) . "&start_date=" . urlencode($start_date) . "&end_date=" . urlencode($end_date);
                                ?>
                                <a href="<?= ($page > 1) ? $base_url . "&page=1" : '#' ?>" class="page-btn <?= ($page <= 1) ? 'disabled' : '' ?>"><i class="fas fa-angle-double-left"></i></a>
                                <a href="<?= ($page > 1) ? $base_url . "&page=" . ($page - 1) : '#' ?>" class="page-btn <?= ($page <= 1) ? 'disabled' : '' ?>">PREV</a>
                                
                                <div class="page-numbers" style="display: flex; gap: 4px;">
                                    <?php for($i = 1; $i <= $total_pages; $i++): 
                                        if($i == 1 || $i == $total_pages || ($i >= $page - 1 && $i <= $page + 1)): ?>
                                            <a href="<?= $base_url ?>&page=<?= $i ?>" class="num-btn <?= ($page == $i) ? 'active' : '' ?>"><?= $i ?></a>
                                ...
                                <a href="<?= ($page < $total_pages) ? $base_url . "&page=" . ($page + 1) : '#' ?>" class="page-btn <?= ($page >= $total_pages) ? 'disabled' : '' ?>">NEXT</a>
                                <a href="<?= ($page < $total_pages) ? $base_url . "&page=" . $total_pages : '#' ?>" class="page-btn <?= ($page >= $total_pages) ? 'disabled' : '' ?>"><i class="fas fa-angle-double-right"></i></a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="details-panel" style="flex: 1; background: white; overflow-y: auto;">
                        <div class="content-wrapper" id="panel-content" style="text-align:center; padding: 100px 50px;">
                            <i class="fas fa-envelope-open" style="font-size: 4rem; opacity: 0.1; margin-bottom: 20px;"></i>
    ...
    function viewDetails(n, element) {
        ...
        const formattedDate = isNaN(dateRaw.getTime()) ? '--' : (dateRaw.getMonth() + 1).toString().padStart(2, '0') + '/' + dateRaw.getDate().toString().padStart(2, '0') + '/' + dateRaw.getFullYear();

        // Safe Fallback String manipulation since there is no join in native notifications array
    ...
    function markAllAsRead() {
        if (confirm("Mark all as read?")) {
            fetch('php/mark_all_read.php').then(() => window.location.reload());
        }
    }
